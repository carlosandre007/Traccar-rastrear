...
const DevicesPage = () => {
  ...
  const baseColumns = 8; 
  const totalColumns = baseColumns + 1 + (manager ? 1 : 0); // +1 para Placa, +1 para Usuários (se manager)
      ...
      <Table className={classes.table}>
        <TableHead>
          <TableRow>
            ...
            <TableCell>{t('sharedPhone')}</TableCell>
            <TableCell>{t('deviceModel')}</TableCell>
            {/* NOVA COLUNA: PLACA DO VEÍCULO */}
            <TableCell>{t('licensePlate')}</TableCell> {/* Reutiliza a chave de tradução da Placa */}
            <TableCell>{t('deviceContact')}</TableCell>
            <TableCell>{t('userExpirationTime')}</TableCell>
            <TableCell>{t('attributeLineNumber')}</TableCell>
            {manager && <TableCell>{t('settingsUsers')}</TableCell>}
            <TableCell className={classes.columnAction} />
          </TableRow>
        </TableHead>
        <TableBody>
          {!loading ? items.filter(filterByKeyword(searchKeyword)).map((item) => (
            <TableRow key={item.id}>
              ...
              <TableCell>{item.phone}</TableCell>
              <TableCell>{item.model}</TableCell>
              <TableCell>{item.attributes?.dadosVeiculo?.placa || ''}</TableCell> {/* Acessa o atributo aninhado */}
              <TableCell>{item.contact}</TableCell>
              <TableCell>{formatTime(item.expirationTime, 'date')}</TableCell>
              <TableCell>{item.attributes?.chip?.lineNumber || ''}</TableCell>
              {manager && <TableCell><DeviceUsersValue deviceId={item.id} /></TableCell>}
              <TableCell className={classes.columnAction} padding="none">
                <CollectionActions
                  itemId={item.id}
          ...
          )) : (<TableShimmer columns={totalColumns} endAction />)}
        </TableBody>
        <TableFooter>
          <TableRow>
            <TableCell>
            ...
            {/* Ajusta o colSpan para o novo número total de colunas */}
            <TableCell colSpan={totalColumns} align="right">
              <FormControlLabel
                control={(
                  <Switch
                    value={showAll}
